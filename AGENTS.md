...
- **Sep 25 Customer briefing people summary:** The Company details rail no longer prints every tracked person's name in the collapsed card. It shows a compact photo/initials stack and count; expanding the dropdown reveals the full clickable rows and Add person action. Verified on the local real-mode Moderna briefing that the dropdown opens/closes and a person row opens the profile/posts dialog. Typecheck, focused lint, and diff check pass. Local only, not deployed.

- **Sep 25 Customer Intel source links:** In the table's Latest intelligence column, some publisher labels were plain text because the homepage resolver could not recognize their names, despite valid story URLs. Publisher labels now link directly to their saved story URL, matching the headline, with a visible blue link cue. Labels without a safe URL remain plain text. The local Customer Intel table shows Bajaj Broking and other previously unlinked sources as links; typecheck, focused lint, and diff check pass. Local only, not deployed.

- **Sep 25 Offering owner hover target:** List-view owner cards had a 130px-wide hover trigger around a 65px avatar-and-name label because a flex-column parent stretched the child across the Owner cell. The owner stack now aligns its children to the start, limiting each trigger to its visible label. In the local Offerings page, the Anir trigger and label both measured 64.5px after the fix; clicking the former blank area did not open the card, while clicking the visible name did. The earlier pointer-motion workaround was removed. Typecheck and diff check pass; focused lint has only three pre-existing unused-symbol warnings. Local only, not deployed.
...
### Sep 25: Agent goal charts and conversation reliability

The full Agent and floating chat now use the same one-retry streaming client for transient failures before any answer text arrives. A failed request remains a retryable alert instead of becoming a false saved assistant message. The floating chat stays open when the user clicks the page, preserves its conversation when opening the full chat, and does not navigate away mid-reply. Goal retrieval exposes exact fiscal-month verified, waiting, and sent-back values; an unset monthly target remains unset. A date-only result on the first of a month stays in that month across time zones. Agent charts can use the Goals page progress rail, display exact comma-separated values with their currency, and reject a target incorrectly mixed into a composition chart. Cross-page answers can retain linked opportunities and solutioning requests. Five bounded Mock-mode Agent questions and focused checks were run locally; see `docs/agent-coverage-audit-2026-09-25.md` for the observed limits. Completed locally, not deployed.
...
### Sep 25: Agent Sessions source and exact links

The Mock Agent guessed a Halcyon pitch session's outcome and review state, then minted a nonexistent session ID. It now reads Sessions records under module permission, with the same latest-contact outcome and review source as the Sessions table. Named-session grounding supplies the exact service, status and link ahead of answer generation. The repeated side-chat answer matched Interested, Regulatory Submission Services and Changes requested; its `/sessions/sess-fill-008` link opened the record. Typecheck, 20 focused workspace tests, lint and diff check pass. The linked detail exposed a separate stale pitch-script issue; see `docs/agent-coverage-audit-2026-09-25.md` for the next cursor. Completed locally, not deployed.

### Sep 25: Mock session pitch identities

The Halcyon session detail reused BioNex's Dr. Mehta pitch across its script, email, and call formats. The Mock store now replaces only untouched sample or generated pitch text using the session's linked customer, contact, and primary service; saved edits with version history are preserved, and the existing durable store migrates without reseeding unrelated Mock work. The generator also removes duplicate sender wording and awkward company possessives. The existing localhost tab showed Halcyon Biosciences and Arjun Duarte across the three formats. Seven focused Mock-density tests, typecheck, lint, and diff check pass. The next Agent coverage target is Tasks; see `docs/agent-coverage-audit-2026-09-25.md`. Completed locally, not deployed.

### Sep 25: Agent Tasks grounding and date-only follow-ups

The Agent initially treated the Tasks page's generic “Needs review” label as the saved state and falsely said Arjun Duarte had no Halcyon follow-ups. A permission-gated Tasks reader now returns the exact pitch review state, each distinct dated interaction, and the actual session/contact links. The repeated Mock side-chat answer matched one “Changes requested” review and three follow-ups; both links opened the correct records, and opening full chat preserved the answer. Calendar-only follow-up dates were also shifted through UTC, making a task due September 25 appear overdue on September 25. Tasks and contact timelines now compare calendar days; the browser shows “Due today” and “today,” and the overdue count changed from 16 to 8. Twenty-two focused tests, typecheck, focused lint and diff check pass. Campaigns or Sequences is the next coverage target; see `docs/agent-coverage-audit-2026-09-25.md`. Completed locally, not deployed.

### Sep 25: Agent Campaigns exact delivery and links

For the queued Regulatory Intelligence pilot invite, the Agent gave correct delivery counts but linked only the Campaigns dashboard. A permission-gated Campaigns reader now exposes the page's exact saved status, recipient, sent, queued, opened and replied counts with the specific campaign URL. It marks seeded Mock delivery as demonstration data. A repeated side-chat answer gave 5 recipients, 2 sent, 3 queued, 1 open and 0 replies and linked `/campaigns/camp-seed-002`; clicking the link opened the matching Mock campaign detail page. Twenty-two focused workspace tests, typecheck, focused lint and diff check pass. Next coverage target: Sequences; see `docs/agent-coverage-audit-2026-09-25.md`. Completed locally, not deployed.

### Sep 25: Agent Sequences records and selection links

An otherwise accurate Regulatory Exec Outreach answer linked a nonexistent `/sequences/seq-exec-outreach` page. The Sequences page now accepts `?sequence=<saved ID>` to select the exact library record. A permission-gated Agent reader returns the saved status, owner, cadence steps, duration, page-matched enrollment count and selection URL; missing enrollment-source access returns null rather than zero. The repeated Mock side-chat answer linked `/sequences?sequence=reg-exec`, which opened the selected seven-step, 18-day sequence with 45 enrolled accounts. A second URL check selected the non-default Intelligence pilot sequence. Twenty-three focused workspace tests, typecheck and diff check pass; lint has no errors and one existing SequencesView warning. Next cursor: non-default or paused sequence, then other uncovered modules. See `docs/agent-coverage-audit-2026-09-25.md`. Completed locally, not deployed.

### Sep 25: Agent Forecast source and fixed reference

The full Agent chat correctly described and linked a paused sequence, including its four-step cadence and one enrollment. A later Forecast side answer falsely called the page's fixed $3M benchmark a configured team quota and pointed to Opportunities for figures actually built from pitch sessions. Forecast now has a permission-gated reader for its session-derived estimates, stage weights, source links and synthetic Mock rep caveat. The page and CSV call the $3M value a fixed reference. A repeated side-chat answer matched $13.7M weighted commit, $36.0M best case, and 94 open pitch-session deals; its Helix Biologics deal link opened the matching $600K Meeting Booked record. Twenty-four focused tests, typecheck, focused lint and diff check pass. Next cursor: Pipeline, then Contacts and Voice. See `docs/agent-coverage-audit-2026-09-25.md`. Completed locally, not deployed.

### Sep 25: Agent Pipeline exact records and estimate labeling

The Agent's Pipeline answer had correct aggregate counts but fabricated links for two tied Xantha Biopharma deals; one link landed on Sessions, and the contact Ravi Almeida was wrongly called the owner. A permission-gated Pipeline reader now returns page-matched stage counts and values, size-derived estimate disclosure, saved owners, and exact deal URLs sorted by value. The repeated Mock side answer gave 94 open deals worth $36,045,000 plus two Closed Lost, and identified the two $1,065,000 Xantha deals as Unassigned. Both links opened their distinct Prospect and Engaged detail records. Pipeline page copy now labels values as estimates and clarifies its reconstructed timeline. Twenty-five focused tests, typecheck, focused lint and diff check pass. Next cursor: Contacts, then Voice; see `docs/agent-coverage-audit-2026-09-25.md`. Completed locally, not deployed.

### Sep 25: Agent Contacts interaction pairs and links

The first full-chat answer for Arjun Duarte at Halcyon Biosciences swapped follow-up dates between No Response and AI Call Completed, though the contact detail showed each date on its own interaction. A permission-gated Contacts reader now supplies each saved outcome, timestamp and follow-up date as one object, exact contact URLs, and authorized linked Pipeline deal URLs. Named-contact questions are grounded in that record. The repeated Mock full-chat question correctly paired all three outcomes and dates and linked the $265,000 Qualified estimate; clicking the contact and deal links opened their exact detail pages in the existing tab. Contacts have no owner field, and linked customer/deal data stays conditional on access. Twenty-six focused workspace tests, typecheck, focused lint and diff check pass. Next cursor: Voice, then remaining uncovered routes; see `docs/agent-coverage-audit-2026-09-25.md`. Completed locally, not deployed.
