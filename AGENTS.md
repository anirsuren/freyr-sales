...
Every graph popup is interactive by default in the shared chart layer, on compact cards as well as expanded charts. Leaving a point, bar, or slice starts a 1.2-second bridge instead of closing immediately; entering the popup cancels that close so the card and its full record list can be read and scrolled. Custom graph HoverCards use the same bridge and no longer disable pointer events on chart-mark popups. Completed locally, not deployed.
