...
Compact group-member avatar stacks fan into named avatar chips on hover and keyboard focus, letting people identify the roster without opening its disclosure. Missing portraits continue to use honest initials; never invent or reuse another person's photograph merely to avoid an initials fallback. In the expanded Organization → group → person modal, clicking a logged result opens its facts, proof, and timeline inline inside the People column rather than launching a second modal behind the first. Completed locally, not deployed.
