...
Every create/add dialog marks each mandatory field with the shared red `RequiredMark` star. Conditional requirements show the star only while that field is required (including either/or fields). The same predicate must govern the visible marker, native `required` attributes where applicable, disabled progression or submit actions, client-side save guards, and API validation. A field may never look optional and then block creation, or look mandatory while creation accepts it empty. Optional fields remain explicitly optional where ambiguity is likely. Apply this rule to every asset and data type in Mock and live modes.
