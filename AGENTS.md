...
- **Sep 24 Lead-origin Solutioning requests:** The request chooser and form now show “For this lead” with the lead person and company. A linked Customer is preselected when present; otherwise the lead's company is selected without creating an account. For a lead-only company, the lead person appears as the contact instead of an empty Customer POC picker. Any recorded lead interest is visible beside the required Solutioning brief, which still must be written. New requests persist `leadRef`/`leadName` separately from the brief, and their detail page links back to the filtered, expanded lead. Checked the local Albert Bourla chooser and meeting form, plus direct lead navigation; typecheck and focused lint pass (one existing unused-prop warning). Local only, not deployed, per the user's current release instruction.
