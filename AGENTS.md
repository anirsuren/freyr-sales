...
### Sep 14: every mock opportunity demonstrates every released tab
The generated mock floor now attaches one solution request, one meeting request, one submission, one presentation, two meetings (one completed and one planned), one contract with schedule/documents, and a populated accrual plan to every generated opportunity. Generation 5 replaces stale generated relationships automatically while preserving hand-created mock rows. Every generated Solutioning item fills all four document shelves with assigned, versioned, openable files and useful notes. Generated opportunity and contract team keys match the generation-stamped record ids, and the team store migrates stale generated keys correctly. Coverage audit: 330 opportunities, 1,320 Solutioning records, 660 meetings, 330 contracts, 800 team assignments, zero relationship gaps. Code/CLI verification only at the user's request.
