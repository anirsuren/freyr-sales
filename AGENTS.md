...
- **Sep 24 Company navigation without an account:** Company names in Leads, both lead analytics workspaces, Opportunities, and Solutioning now open the customer account when one exists. Otherwise they open a read-only company page with matching leads, deals, and Solutioning items, without creating a customer account. The page reads only modules the viewer may open; Solutioning member visibility matches its list, and the page redirects to a customer account only when Customers access is available. Typecheck, focused lint (existing unused-symbol warnings only), and diff check pass. Local only, per the user's release instruction.
...
- **Sep 24 Lead-origin Solutioning requests:** The request chooser and form show the lead person's name and company, never the lead ID. A linked Customer is preselected when present; otherwise the lead's company is selected without creating an account. For a lead-only company, the lead person appears as the contact instead of an empty Customer POC picker. Any recorded lead interest is visible beside the required Solutioning brief, which still must be written. New requests persist `leadRef`/`leadName` separately from the brief, and their detail page shows the person's name while using the ID only in its link back to the filtered, expanded lead. Checked the local Albert Bourla chooser and meeting form, plus direct lead navigation; typecheck and focused lint pass (one existing unused-prop warning). Local only, not deployed, per the user's current release instruction.
...
- **Sep 24 Lead form Company field alignment:** When entering a company name that is not in the account list, the Company text box now fills its entire form column like Person and the other fields. “Pick from list” sits inside the right end of that box, and the lead-only explanation moved to the field's help hint, so the next row stays aligned. Typecheck and focused lint pass (one existing unused-prop warning); diff check passes. Local only, not deployed.
