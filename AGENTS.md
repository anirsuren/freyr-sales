...
Every graph popup is interactive by default in the shared chart layer, on compact cards as well as expanded charts. Leaving a point, bar, or slice starts a 1.2-second bridge so the cursor can reach the popup; entering the popup cancels that close so the card and its full record list can be read and scrolled. Once the cursor leaves the popup itself, close it immediately. Custom graph HoverCards follow the same mark-to-popup bridge and immediate popup-exit rule, and chart-mark popups keep pointer events enabled. Completed locally, not deployed.
