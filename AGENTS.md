...
The generated mock floor now attaches one solution request, one meeting request, one submission, one presentation, two meetings (one completed and one planned), one contract with schedule/documents, and a populated accrual plan to every generated opportunity. Generation 4 replaces stale generated relationships automatically while preserving hand-created mock rows. Generated opportunity and contract team keys now match the generation-stamped record ids, and the team store migrates stale generated keys correctly. Coverage audit: 330 opportunities, 1,320 Solutioning records, 660 meetings, 330 contracts, 800 team assignments, zero relationship gaps. Code/CLI verification only at the user's request.
