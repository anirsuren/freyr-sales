...
Customer Intel now includes “Compliance & Enforcement” between RA/QA Team Changes and Technology Changes. It only tags records published at verified health-authority URLs, never customer releases or press coverage. A bounded daily pass gathers official FDA drug, device, and food recall records plus MHRA drug/device alerts, with official-domain discovery for FDA, EMA, MHRA, TGA, Health Canada, HSA, and MDA notices. The company briefing separates these under a Health authorities source filter and marks each card with the issuing authority. Read-only checks found current FDA recall records for Amgen and an MHRA defect notice for B. Braun; classification/source tests, typecheck, and focused lint pass. Completed locally, not deployed.
