...
Every create/add dialog marks each mandatory field with the shared red `RequiredMark` star. Conditional requirements show the star only while that field is required (including either/or fields). The same predicate governs the visible marker, native `required` attributes where applicable, disabled progression or submit actions, client save guards, and API validation. Apply this rule to every asset and data type in Mock and live modes.
