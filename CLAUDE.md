...
Goal progress rails use one consistent status palette throughout the Goals module: verified portions are solid green, waiting-for-verification portions are striped amber/yellow, and sent-back portions are striped red. Waiting pills and labels use the same amber treatment.

When one goal bar contains multiple workflow statuses, render each status as its own proportional segment and keep the visible stack consistent with the tooltip total. Waiting-for-verification is striped amber and sent-back is striped red.
...
Every create, add, edit, log, upload, and configuration dialog labels every editable data field as either required or optional. Required fields use the shared red `RequiredMark` star; optional fields use the shared `OptionalMark` text. The shared `Field` component defaults to optional, so a new field cannot silently omit its state; use `requirement="none"` only for read-only facts or action controls. Conditional requirements switch between those two states using the same predicate that governs native `required` attributes where applicable, disabled progression or submit actions, client save guards, and API validation. Apply this rule to every asset and data type in Mock and live modes.
