    ...
    <div>
      <div className="flex items-start justify-between gap-4 mb-6">
        <div className="flex items-center gap-4">
          <Avatar name={customer?.company_name || "?"} className="w-12 h-12 text-[16px] rounded-xl" />
          <div>
      ...
      <Card className="mb-6">
        ...
        <div className="flex items-start gap-2 overflow-x-auto">
          {STAGES.map((s, i) => {
            ...
            return (
              <div key={s} className="flex items-start gap-2 shrink-0">
                ...
                {i < STAGES.length - 1 && (
                  <span className={cn("w-6 h-px mt-3.5", done ? "bg-blue-primary" : "bg-border-light")} />
                )}
              </div>
            );
          })}
