...
Module._load=function(id,parent,...rest){if(id==='@/lib/marketIntelRead')return{readMarketIntelFeed:async()=>({companies:{example:company},people:{}}),buildBriefing:()=>({signals:[]})};if(id==='@/lib/marketIntelTracking')return{readMarketIntelTracking:async()=>({companies:[],people:[]})};return load.call(this,id,parent,...rest);};
...
test('unscoped retrieval retains older records with explicit coverage limits',async()=>{const result=await searchMarketIntel('Example Corp');assert.match(result,/Old post/);assert.match(result,/Old website item/);assert.match(result,/displayed subset is never the total/);});
