...
/**
 * A COMPONENT'S FEATURES ARE PART OF ITS ROADMAP (Anir, Aug 21, relaying the
 * product owner: "also there are changes in the features — someone added new
 * features, someone deleted new features, so that all the time needs").
 *
 * The first cut watched only the version rows, so a save that silently
 * dropped a feature out of V2 minted nothing and told nobody. Only the fields
 * that ARE the promise are watched: the name, and which versions carry it.
 * Descriptions and attachments move for a hundred harmless reasons.
 */
...
export function describeComponentChange(
  ...
  featuresBefore: ComponentFeature[] = [],
  featuresAfter: ComponentFeature[] = []
): string[] {
  const changes: string[] = [];
  ...
  /* THE FEATURE LINES. Named by the feature, and where a feature moves
     between versions the line says which way, because "V2 lost Bulk export"
     is the sentence somebody has to answer for to a customer. */
  const fKey = (f: ComponentFeature) => (f.name || "").trim().toLowerCase();
  const fPrev = new Map(featuresBefore.map((f) => [f.id, f]));
  const fNext = new Map(featuresAfter.map((f) => [f.id, f]));
  const versionName = (id: string) =>
    after.find((r) => r.id === id)?.version ||
    before.find((r) => r.id === id)?.version ||
    "a version";
  for (const f of featuresAfter) {
    if (!fPrev.has(f.id)) changes.push(`Added the feature ${f.name || "(unnamed)"}`);
  }
  for (const f of featuresBefore) {
    if (!fNext.has(f.id)) changes.push(`Removed the feature ${f.name || "(unnamed)"}`);
  }
  for (const f of featuresAfter) {
    const was = fPrev.get(f.id);
    if (!was) continue;
    if (fKey(was) !== fKey(f))
      changes.push(`Renamed the feature ${was.name} to ${f.name}`);
    const had = new Set(was.versionIds ?? []);
    const has = new Set(f.versionIds ?? []);
    for (const id of has)
      if (!had.has(id)) changes.push(`${f.name} is now in ${versionName(id)}`);
    for (const id of had)
      if (!has.has(id)) changes.push(`${f.name} is no longer in ${versionName(id)}`);
  }
  if (
    !changes.length &&
    componentRoadmapChanged(before, after, featuresBefore, featuresAfter)
  )
...
export function componentRoadmapChanged(
  ...
  if (norm(before) !== norm(after)) return true;
  /* Only name and version mapping: a typo fixed in a description is not a
     roadmap change and must not mint a version anybody has to read. */
  const normF = (fs: ComponentFeature[]) =>
    stable(
      [...fs]
        .map((f) => ({
          id: f.id,
          name: (f.name || "").trim(),
          versionIds: [...(f.versionIds ?? [])].sort(),
        }))
        .sort((a, b) => a.id.localeCompare(b.id))
    );
  return normF(featuresBefore) !== normF(featuresAfter);
}
...
export function nextComponentVersions(
  ...
  /* A save that sends only features leaves the versions exactly as they were,
     so the release list falls back to what is already stored rather than to
     an empty array that would read as "every version removed". */
  const b = after.releases ?? a;
  const fa = before.features ?? [];
  const fb = after.features ?? fa;
  if (!componentRoadmapChanged(a, b, fa, fb)) return null;
  const history = before.roadmap_versions ?? [];
  const last = history.reduce((max, v) => Math.max(max, v.version || 0), 0);
  return [
    {
