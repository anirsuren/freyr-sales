 ...
 * THE PREVIEW ITSELF
 *
 * Lifted out of the route so a SOLUTIONING document renders exactly the way a
 * sales material does (Anir, Aug 26: "if the customer documents are the sales
 * material... copy all that shit. Every single part of it, like the preview,
 * the hover"). The route above answers "may this person see this file"; this
 * answers "what does this file look like", and both callers get the same
 * answer because it is the same code.
 * ------------------------------------------------------------------ */

type PreviewPayload = { body: unknown; status?: number };

const payload = (body: unknown, init?: { status?: number }): PreviewPayload => ({
  body,
...
export async function buildMaterialPreview({
  ...
  path: string;
  /** A member inside a ZIP, when the reader has opened one. */
  member: string | null;
  /** Where the raw bytes are served from, for the formats a browser renders. */
  inlineUrl: string;
  ...
  try {
    ...
    if (buffer.byteLength > MAX_CONVERT_BYTES) {
      return payload({
        ...
        label: member ? member.split("/").pop() || member : label,
      });
    }
