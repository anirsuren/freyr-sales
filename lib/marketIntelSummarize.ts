...
${group === "customer" ? `  "events" includes the company's own named summits, conferences, webinars and patient-advocacy gatherings. A company post reporting what happened at its summit is an event even if it is not specifically about regulatory affairs; do not put it under "others".` : ""}
${group === "customer" ? `  "financial_operational" is for changes to the company's actual earnings, revenue, guidance, spending, production, capacity or supply chain. A stock/share price movement, trading-volume report, analyst rating or price-target change alone is market commentary: label it "others", not "financial_operational". If a stock-focused article also reports a separate substantive company development, classify that development under its appropriate signal.` : ""}
