...
const sections: { id: ReviewSection; label: string; icon: typeof Target }[] = [
  { id: "decision", label: "Decision brief", icon: Target },
  { id: "people", label: "People & influence", icon: UsersRound },
  { id: "actions", label: "Next moves", icon: CheckCircle2 },
];
...
export function OpportunityReviewView({ review, mayEdit, dealId }: { review?: OpportunityReview; mayEdit: boolean; dealId: string }) {
  ...
  return <div className="pb-12">
    ...
    <div role="tablist" aria-label="Opportunity review sections" className="mt-5 flex w-fit max-w-full gap-1 overflow-x-auto rounded-xl bg-surface-secondary p-1">
      {sections.map(({ id, label, icon: Icon }) => <button key={id} type="button" role="tab" id={`review-tab-${id}`} aria-selected={section === id} aria-controls={`review-panel-${id}`} onClick={() => setSection(id)} className={`inline-flex shrink-0 items-center gap-2 rounded-lg px-3.5 py-2 text-[12.5px] font-semibold transition-colors ${section === id ? "bg-white text-blue-primary shadow-sm" : "text-text-secondary hover:text-text-primary"}`}><Icon size={15} />{label}{id === "people" && people.length > 0 && <span className="rounded-full bg-blue-50 px-1.5 text-[10px] text-blue-primary">{people.length}</span>}{id === "actions" && !!data?.actions.length && <span className="rounded-full bg-blue-50 px-1.5 text-[10px] text-blue-primary">{data.actions.length}</span>}</button>)}
    </div>

    {section === "decision" && <div role="tabpanel" id="review-panel-decision" aria-labelledby="review-tab-decision" className="mt-5 space-y-4">
    ...
    {section === "people" && <div role="tabpanel" id="review-panel-people" aria-labelledby="review-tab-people" className="mt-5">
    ...
    {section === "actions" && <div role="tabpanel" id="review-panel-actions" aria-labelledby="review-tab-actions" className="mt-5 grid gap-4 lg:grid-cols-[minmax(0,1.3fr)_minmax(290px,0.7fr)]">
