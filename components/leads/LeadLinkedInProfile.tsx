...
function ProfileDetails({ profile, url }: { profile: LeadLinkedInProfile; url: string }) {
  ...
  const latestPost = [...profile.recentPosts].sort((a, b) => (Date.parse(b.date ?? "") || 0) - (Date.parse(a.date ?? "") || 0))[0];
  return <div className="space-y-4">
    ...
    {latestPost && <Section title="Latest public activity">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <span className="text-[11px] text-text-tertiary">{day(latestPost.date) || "Recent post"}</span>
        <a href={latestPost.url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 text-[11px] font-semibold text-blue-primary hover:underline">Open post <ExternalLink size={11} /></a>
      </div>
      <p className="mt-2 line-clamp-3 whitespace-pre-wrap text-[12.5px] leading-5 text-text-secondary">{latestPost.text}</p>
    </Section>}

    <details className="rounded-xl border border-border-light bg-white">
      <summary className="cursor-pointer px-4 py-3 text-[12px] font-semibold text-text-primary">Career and background · {profile.experience.length} roles</summary>
      {(profile.followerCount !== undefined || profile.connectionCount !== undefined) && <p className="border-t border-border-light px-4 py-2 text-[11px] text-text-tertiary">{[profile.followerCount !== undefined ? `${profile.followerCount.toLocaleString()} followers` : "", profile.connectionCount !== undefined ? `${profile.connectionCount.toLocaleString()} connections` : ""].filter(Boolean).join(" · ")}</p>}
      ...
      {profile.skills.length > 0 && <div className="border-t border-border-light p-4"><Section title={`Skills · ${profile.skills.length}`}><div className="flex flex-wrap gap-1.5">{profile.skills.map((skill, index) => <span key={`${skill}-${index}`} className="rounded-full border border-border-light bg-surface px-2.5 py-1 text-[11px] font-medium text-text-secondary">{skill}</span>)}</div></Section></div>}
    </details>

    <details className="rounded-xl border border-border-light bg-white">
      <summary className="cursor-pointer px-4 py-3 text-[12px] font-semibold text-text-primary">All public posts · {profile.recentPosts.length}</summary>
      <div className="border-t border-border-light p-4"><Section title="LinkedIn posts">
  ...
  return <>
    <div className="mt-4 rounded-xl border border-border-light bg-surface p-3.5">
      ...
      {profile && <div className="mt-3 border-t border-border-light pt-3">
        ...
        <button type="button" onClick={(event) => { event.stopPropagation(); setOpen(true); }} className="mt-2 text-[12px] font-semibold text-blue-primary hover:underline">View profile details →</button>
      </div>}
    </div>
