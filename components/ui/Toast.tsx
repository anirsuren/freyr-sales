...
export function ToastProvider({ children }: { children: React.ReactNode }) {
  ...
  return (
    <ToastCtx.Provider value={{ toast }}>
      {children}
      {/* Sits ABOVE the agent dock bubble (which owns the bottom-right
          corner) — toasts were sliding underneath it (Anir, Aug 11: "the AI
          system always covers that up"). */}
      <div className="fixed bottom-[88px] right-6 z-[100] flex flex-col gap-2 pointer-events-none">
        {toasts.map((t) => (
          <div
            key={t.id}
