...
// The animated body is the ONLY part that needs client state, so it lives in
// its own "use client" module. Marking this whole card client-side broke every
// server page that passes an `icon` — a component reference cannot be sent
// across the boundary ("Only plain objects can be passed to Client Components").
// `children` still render on the server and pass through untouched.
